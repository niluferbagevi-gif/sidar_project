...
# """
# Sidar Project - Web Arayüzü Sunucusu
# FastAPI + SSE (Server-Sent Events) ile akış destekli chat arayüzü.
...
#     """
#     Kullanıcı mesajını SSE akışı olarak işler.
#     agent.respond() senkron generator olduğu için
#     thread + queue köprüsü ile async SSE'ye dönüştürülür.
#     """
#     body = await request.json()
#     user_message = body.get("message", "").strip()
...
#     def producer() -> None:
#         """Senkron thread: ajan yanıtlarını queue'ya besler."""
#         try:
#             for chunk in get_agent().respond(user_message):
#                 chunk_queue.put(chunk)
#         except Exception as exc:
#             logger.exception("Agent respond hatası: %s", exc)
#             chunk_queue.put(f"\n[Sistem Hatası] {exc}")
#         finally:
#             chunk_queue.put(None)  # Sentinel — akış bitti
