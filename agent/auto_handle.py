...
class AutoHandle:
    ...
    def handle(self, text: str) -> Tuple[bool, str]:
        ...
        # ── Web Arama araçları ─────────────────────
        result = self._try_web_search(t, text)
        if result[0]:
            return result

        result = self._try_fetch_url(t, text)
        if result[0]:
            return result

        result = self._try_search_docs(t, text)
        if result[0]:
            return result

        result = self._try_search_stackoverflow(t, text)
        if result[0]:
            return result

        # ── Paket Bilgi araçları ───────────────────
        result = self._try_pypi(t, text)
        if result[0]:
            return result

        result = self._try_npm(t, text)
        if result[0]:
            return result

        result = self._try_gh_releases(t, text)
        if result[0]:
            return result
    ...
    # ─────────────────────────────────────────────
    #  WEB ARAMA İŞLEYİCİLERİ
    # ─────────────────────────────────────────────

    def _try_web_search(self, t: str, raw: str) -> Tuple[bool, str]:
        ...
        if m:
            ...
            _, result = self.web.search(query)
            return True, result
        return False, ""

    def _try_fetch_url(self, t: str, raw: str) -> Tuple[bool, str]:
        ...
        if re.search(r"url.*(oku|çek|getir|fetch)|fetch.*url", t):
            ...
            _, result = self.web.fetch_url(url)
            return True, result
        return False, ""

    def _try_search_docs(self, t: str, raw: str) -> Tuple[bool, str]:
    ...
    def _try_search_stackoverflow(self, t: str, raw: str) -> Tuple[bool, str]:
        """Stack Overflow araması — 'stackoverflow: python async' vb."""
        m = re.search(r"(?:stackoverflow|so)\s*[:\-]\s*(.+)", t)
        if m:
            query = m.group(1).strip()
            _, result = self.web.search_stackoverflow(query)
            return True, result
        return False, ""

    # ─────────────────────────────────────────────
    #  PAKET BİLGİ İŞLEYİCİLERİ
    # ─────────────────────────────────────────────

    def _try_pypi(self, t: str, raw: str) -> Tuple[bool, str]:
        ...
        if m:
            ...
            if ver_m:
                _, result = self.pkg.pypi_compare(package, ver_m.group(1))
            else:
                _, result = self.pkg.pypi_info(package)
            return True, result
        return False, ""

    def _try_npm(self, t: str, raw: str) -> Tuple[bool, str]:
        ...
        if m:
            package = m.group(1).strip()
            _, result = self.pkg.npm_info(package)
            return True, result
        return False, ""

    def _try_gh_releases(self, t: str, raw: str) -> Tuple[bool, str]:
        ...
        if m:
            ...
            _, result = self.pkg.github_releases(repo)
            return True, result
        return False, ""

    # ─────────────────────────────────────────────
    #  RAG / BELGE DEPOSU İŞLEYİCİLERİ
    # ─────────────────────────────────────────────
